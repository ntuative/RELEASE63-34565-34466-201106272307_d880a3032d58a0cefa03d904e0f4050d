package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1720:String = "asset-libraries";
      
      private static const const_1717:String = "library";
      
      private static const const_1716:String = "service-libraries";
      
      private static const const_1722:String = "library";
      
      private static const const_1719:String = "component-libraries";
      
      private static const const_1721:String = "library";
      
      private static const const_1718:String = "error_data";
      
      private static const const_1714:String = "error_cat";
      
      private static const const_1715:String = "error_desc";
       
      
      private var var_423:LibraryLoaderQueue;
      
      private var var_921:IEventDispatcher;
      
      private var var_598:uint;
      
      private var var_424:Function;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         switch(param2 & 0)
         {
            case Core.const_1227:
               debug("Core; using simple frame update handler");
               this.var_424 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_741:
               debug("Core; using complex frame update handler");
               this.var_424 = this.complexFrameUpdateHandler;
               break;
            case Core.const_757:
               debug("Core; using profiler frame update handler");
               this.var_424 = this.profilerFrameUpdateHandler;
               this.var_236 = new Profiler(this);
               attachComponent(this.var_236,[new IIDProfiler()]);
               var_432.addChild(new ProfilerViewer(this.var_236));
               break;
            case Core.const_607:
               debug("Core; using experimental frame update handler");
               this.var_424 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_424 = this.debugFrameUpdateHandler;
         }
      }
      ...
      {
         return this.var_598 - this.getNumberOfFilesPending();
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            try
            {
               ...
               {
                  receivers = this.var_308[i] as Array;
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1718,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1714,String(param3));
         ErrorReportStorage.setParameter(const_1715,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_921 = loadingEventDelegate;
         if(this.var_423 == null)
         {
            this.var_423 = new LibraryLoaderQueue(_debug);
         }
         try
         {
            node = config.child(const_1720)[0];
            if(node != null)
            {
               list = node.child(const_1717);
         ...
         try
         {
            node = config.child(const_1716)[0];
            if(node != null)
            {
               list = node.child(const_1722);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         try
         {
            node = config.child(const_1719)[0];
            if(node != null)
            {
               list = node.child(const_1721);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1199);
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_921)
         {
            this.var_921.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_921)
         {
            ...
            this.var_921.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_921)
                  {
                     this.var_921.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_598 - this.var_423.length,this.var_598,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_236 ? 0 : this.var_1688 & 0;
         if(_loc3_ == Core.const_607)
         {
            this.var_308[param2].push(new UpdateDelegate(param1,this,param2));
         }
         else
         {
            this.var_308[param2].push(param1);
         }
      }
      ...
      override public function removeUpdateReceiver(param1:IUpdateReceiver) : void
      {
         ...
         var _loc3_:* = null;
         var _loc6_:* = null;
         var _loc4_:int = !!this.var_236 ? 0 : this.var_1688 & 0;
         var _loc5_:int = 0;
         while(_loc5_ < CoreComponentContext.const_107)
         {
            _loc3_ = this.var_308[_loc5_] as Array;
            if(_loc4_ == Core.const_607)
            {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_424(_loc2_,_loc2_ - this._lastUpdateTimeMs);
      ...
      private function simpleFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         {
            this.var_739[priority] = 0;
            receivers = this.var_308[priority];
            index = 0;
            length = receivers.length;
            while(index != length)
            {
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         while(priority < CoreComponentContext.const_107)
         {
            ...
            {
               if(this.var_739[priority] < priority)
               {
                  ++this.var_739[priority];
                  skip = true;
               }
            }
            if(!skip)
