package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_865:IFigurePartSet;
      ...
      private var var_687:Rectangle;
      
      private var var_1994:int = 0;
      
      private var var_477:IAvatarRenderManager;
      
      private var var_1142:Boolean;
      
      private var var_1993:BitmapData;
      
      private const const_2368:int = 13421772;
      
      private const const_2369:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this.var_865 = param3;
         this._window = param1;
         this.var_1592 = this._window.findChildByTag("BG_COLOR");
         this._colors = param4;
         this.var_1995 = param5;
         if(param3 == null)
         {
            this.var_751 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_1994 = Math.max(this.var_1994,_loc6_.colorLayerIndex);
            }
         }
         this.var_477 = this.var_78.controller.manager.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_751 != null && !this.var_1995)
            {
               _loc3_ = this.var_751;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_865)
            {
               switch(this.var_865.clubLevel)
               {
                  case HabboClubLevelEnum.const_54:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_38:
                     _loc2_.style = HabboIconType.const_752;
                     break;
                  case HabboClubLevelEnum.const_46:
                     _loc2_.style = HabboIconType.const_981;
               }
            }
            else
            {
         ...
         if(this.var_1592 == null)
         {
            return;
         }
         if(this.isSelected)
         {
            this.var_1592.color = this.const_2368;
         }
         else
         {
            this.var_1592.color = this.const_2369;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_477.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_477.isFigureReady(_loc1_))
         {
            this.var_477.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_390[_loc6_] + "_" + FigureData.const_158;
               _loc4_ = this.var_477.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_687 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1993)
               {
                  ...
                  this.var_1993 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_687.width,this.var_687.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_687.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_687.y;
               if(this.var_1995 && _loc2_.colorLayerIndex > 0)
               {
