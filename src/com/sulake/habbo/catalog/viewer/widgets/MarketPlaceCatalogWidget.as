package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class MarketPlaceCatalogWidget extends CatalogWidget implements ICatalogWidget, IGetImageListener, IMarketPlaceVisualization
   {
      ...
      private const const_2065:int = 2;
      
      private const const_2340:int = 40;
      
      private const const_2339:int = 10;
      
      private var var_761:Array;
      
      private var var_1443:MarketPlaceOfferData;
      ...
      override public function dispose() : void
      {
         ...
         if(this.var_518)
         {
            this.var_518.removeEventListener(TimerEvent.TIMER,this.onPopulationTimer);
            this.var_518 = null;
         }
      }
      ...
      private function updateList() : void
      {
         ...
         if(!this.var_618)
         {
            ...
            this.var_618 = _loc5_.buildFromXML(_loc4_) as IWindowContainer;
         }
         if(!this.var_618)
         {
            return;
         ...
         this.updateStatusDisplay(this.const_2065,_loc3_.length,_loc2_);
         if(!this.var_518)
         {
            this.var_518 = new Timer(25);
            this.var_518.addEventListener(TimerEvent.TIMER,this.onPopulationTimer);
         }
         this.var_1168 = 0;
         this.populateList();
         this.var_518.start();
      ...
      private function populateList() : Boolean
      {
         ...
         while(_loc1_ < 5)
         {
            if(this.var_1168 >= this._offers.length)
            {
               return true;
            }
            this.addListItem(this._offers.getWithIndex(this.var_1168));
            ++this.var_1168;
            _loc1_++;
         }
         return false;
      ...
      private function addListItem(param1:MarketPlaceOfferData) : void
      {
         ...
         var _loc10_:* = null;
         var _loc11_:* = null;
         if(!param1 || !this._itemList || !this.var_618 || !this.marketPlace.localization)
         {
            return;
         }
         var _loc2_:IWindowContainer = this.var_618.clone() as IWindowContainer;
         ...
         else if(param1.type == WindowEvent.const_114)
         {
            ...
            switch(_loc7_.name)
            {
               ...
               case "max_price_input":
                  _loc8_ = this.const_2339;
                  break;
               case "search_input":
                  _loc8_ = this.const_2340;
                  break;
               default:
                  return;
            }
         ...
         if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK)
         {
            switch(param2.name)
            {
               case "back_button":
                  ...
                  break;
               case "buy_button":
                  this.marketPlace.buyOffer(this.var_1443.offerId);
            }
            return;
         }
         if(param1.type == WindowEvent.const_48)
