package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
      ...
      private var var_482:IFriendCategoriesDeps;
      ...
      {
         var _loc2_:int = !!param1.online ? int(param1.categoryId) : int(FriendCategory.const_434);
      ...
      public function onFriendListUpdate(param1:IMessageEvent) : void
      {
         ...
         try
         {
            ...
            for each(updatedFriend in p.updatedFriends)
            {
               ...
               this.var_482.messenger.setFollowingAllowed(updatedFriend.id,updatedFriend.followingAllowed && updatedFriend.online);
               currentlyOnline = this.isFriendOnline(updatedFriend.id);
               if(currentlyOnline && !updatedFriend.online)
               {
                  this.var_482.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
               }
               if(!currentlyOnline && updatedFriend.online)
               {
                  this.var_482.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_482.view.setNewMessageArrived();
               }
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         this.findCategory(FriendCategory.const_434).received = true;
         this.findCategory(FriendCategory.const_1173).received = true;
