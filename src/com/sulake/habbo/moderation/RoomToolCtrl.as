package com.sulake.habbo.moderation
{
   ...
   public class RoomToolCtrl implements IDisposable, TrackedWindow
   {
      ...
      private var var_1284:IDropMenuWindow;
      
      private var var_600:ITextFieldWindow;
      
      private var var_721:Boolean = true;
      
      private var var_1038:ICheckBoxWindow;
      
      private var var_1563:ICheckBoxWindow;
      
      private var var_1562:ICheckBoxWindow;
      ...
      public function show() : void
      {
         ...
         this.var_36.connection.send(new GetModeratorRoomInfoMessageComposer(this.var_390));
         Logger.log("BEGINNING TO SHOW: " + this.var_390);
      }
      
      public function getType() : int
      {
         return WindowTracker.const_1959;
      }
      ...
      {
         Logger.log("GOT ROOM INFO: " + param1.flatId + ", " + this.var_390);
         if(param1.flatId != this.var_390)
         {
            Logger.log("NOT THE SAME FLAT: " + param1.flatId + ", " + this.var_390);
            return;
         }
         this._data = param1;
         this.populate();
      ...
      public function populate() : void
      {
         ...
         this.var_600 = ITextFieldWindow(this._frame.findChildByName("message_input"));
         this.var_600.procedure = this.onInputClick;
         this.var_1284 = IDropMenuWindow(this._frame.findChildByName("msgTemplatesSelect"));
         this.prepareMsgSelect(this.var_1284);
         this.var_1284.procedure = this.onSelectTemplate;
         this.var_1038 = ICheckBoxWindow(this._frame.findChildByName("kick_check"));
         this.var_1563 = ICheckBoxWindow(this._frame.findChildByName("lock_check"));
         this.var_1562 = ICheckBoxWindow(this._frame.findChildByName("changename_check"));
      ...
      private function onChatlog(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_36.windowTracker.show(new ChatlogCtrl(new GetRoomChatlogMessageComposer(0,this._data.flatId),this.var_36,WindowTracker.const_934,this._data.flatId),this._frame,false,false,true);
      }
      ...
      {
         if(this.var_721 || this.var_600.text == "")
         {
            this.var_36.windowManager.alert("Alert","You must input a message to the user",0,this.onAlertClose);
            return;
         }
         var _loc2_:int = this.determineAction(param1,this.var_1038.isSelected);
         this.var_36.connection.send(new ModeratorActionMessageComposer(ModeratorActionMessageComposer.const_1933,_loc2_,this.var_600.text,"",""));
         if(this.var_1563.isSelected || this.var_1562.isSelected || this.var_1038.isSelected)
         {
            this.var_36.connection.send(new ModerateRoomMessageComposer(this._data.flatId,this.var_1563.isSelected,this.var_1562.isSelected,this.var_1038.isSelected));
         }
         this.dispose();
      }
      ...
      private function determineAction(param1:Boolean, param2:Boolean) : int
      {
         if(param2)
         {
            return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1794);
         }
         return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1848);
      }
      ...
      private function onSelectTemplate(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         var _loc3_:String = this.var_36.initMsg.roomMessageTemplates[this.var_1284.selection];
         if(_loc3_ != null)
         {
            this.var_721 = false;
            this.var_600.text = _loc3_;
         }
      }
   }
}
