package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_1363:Number = 1.0;
      
      private var var_723:BitmapData;
      
      private var var_905:ColorTransform;
      
      private var var_1666:ColorTransform;
      
      private var var_2143:Matrix;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_905 = new ColorTransform();
         this.var_905.alphaMultiplier = 1;
         this.var_1666 = new ColorTransform();
         this.var_2143 = new Matrix();
      ...
      {
         var _loc1_:* = null;
         for each(_loc1_ in this.var_1362)
         {
            _loc1_.dispose();
         }
         this.var_1362 = null;
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_116 = this.var_1362[param1];
         this.var_1121 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_2142 >= 0)
         {
            this.var_300 = this._visualization.getSprite(this.var_2142);
            if(this.var_300 && this.var_300.asset)
            {
               ...
               {
                  this.var_100 = this.var_300.asset.clone();
                  if(this.var_905.alphaMultiplier != 1)
                  {
                     this.var_723 = new BitmapData(this.var_100.width,this.var_100.height,true,4278190080);
                  }
               }
               this.var_2559 = -this.var_300.offsetX;
               this.var_2560 = -this.var_300.offsetY;
               this.var_300.asset = this.var_100;
            }
            if(this.var_100)
            {
            ...
            if(this.var_723)
            {
               this.var_723.fillRect(this.var_723.rect,4278190080);
            }
         }
      }
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_116.y * this.var_1363;
         }
         return 0;
      }
      ...
      public function updateSprites() : void
      {
         ...
         if(this.var_100 && this.var_300.asset != this.var_100)
         {
            this.var_300.asset = this.var_100;
         }
         if(this.var_1121)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_1121 && this.var_116.hasIgnited)
         {
            this.var_1121 = true;
         }
         _loc8_ = this.var_1171 * this.var_1363;
         this.var_116.update();
         if(this.var_1121)
         {
            if(this.var_116.roomObjectSpriteId >= 0)
            ...
            {
               this.var_100.draw(this.var_723,this.var_2143,this.var_905,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_116.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2559 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1363;
               _loc4_ = this.var_2560 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1363;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1666.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_100.draw(_loc7_,this._translationMatrix,this.var_1666,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2954 = parseInt(param1.@size);
         this.var_2142 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1171 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1363 = this.var_2954 / 64;
         this._blend = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this._blend = Math.min(this._blend,1);
         this.var_905.alphaMultiplier = this._blend;
